...
- **QC-1** — `selectPlacementRelay` ignores `canaryHealthy` → **MUST-FIX-BEFORE-M4b** (a placement exclusion is request-side; M3 narrows the candidate set via `excludeFlaggedRelays` but does NOT re-open QC-1 inside the scorer).
