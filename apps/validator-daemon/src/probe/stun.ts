/**
 * STUN protocol implementation for the Validator daemon's real network probing.
 *
 * Send STUN Binding Requests via UDP to measure RTT, jitter, and packet loss.
 * All numeric results use bigint (basis-point invariant -- never floating point).
 *
 * Follows RFC 5389 STUN Binding Request format.
 *
 * Extracted from the former `probe.ts` monolith (STUN half); `probe.ts` re-exports
 * {@link StunProbeResult} and {@link stunProbe} so external import sites are unchanged.
 */
