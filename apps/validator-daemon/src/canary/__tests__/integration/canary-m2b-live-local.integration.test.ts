/**
 * M2b-live N6 — REQ-MLL-01/03/04/05/06/10. The GATED cross-process e2e: a relay+orchestrator parent
 * (this test) drives a real Node mediasoup producer through the additive byzantine evil-relay forward
 * over a REAL router->router F1 PipeTransport to TWO INDEPENDENT validator Node child processes. Each
 * child is its own OS process (a real mediasoup worker + the UNCHANGED runCanaryVerifyRound + its own
 * Wallet-B + a loopback claims-server). The children cross-post their Wallet-B self-attestations to
 * each other over the loopback claims-server, so >=2-distinct is the product of two SEPARATE processes
 * (NOT one process holding two keypairs) — closing the M2b "co-homing simulated" caveat for the local
 * case.
 *
 *   HONEST forward    -> 0 proofs on both children (INV-A, no false positive).
 *   BYZANTINE forward -> both detect from the captured bytes -> a >=2-distinct proof (REQ-MLL-05).
 *
 * Run: cd apps/validator-daemon && npx vitest run --config vitest.canary.config.ts canary-m2b-live-local
 */
...
describe('REQ-MLL-01/03/04/05/06/10 — cross-process real-media canary detection', () => {
