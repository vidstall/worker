...
export interface LiveConsumerRuntime extends LiveConsumer {
   ...
   * cross-host WAN hop; on the flag-OFF loopback path the srtpParameters key is omitted (spread
   * collapses to {})). Reuses the canonical `PipeConnectParams` shape from @dvconf/inter-relay-client.
   */
...
export async function bringUpLiveConsumer(args: {
  ...
  return {
    standbyParams: {
      ...
      // the relay primary can connect cross-host. `!== undefined`-guarded so flag-OFF omits the field
    ...
    async connectToRelay(relay): Promise<void> {
      await standbyPipe.connect({
        ...
        // `!== undefined`-guarded so the key is omitted on the flag-OFF loopback path and connect()
        // behaves byte-identically.
        ...(relay.srtpParameters !== undefined ? { srtpParameters: relay.srtpParameters } : {}),
