/**
 * Validator Daemon -- named setup-phase functions extracted from `startDaemon`'s body.
 *
 * `bootstrap.ts` keeps `startDaemon`'s orchestration skeleton and calls into these:
 *   - `registerAndSetupSessionWallet` -- auto-register + generate/register/fund the session wallet.
 *   - `startCanaryCellLoopAndCoverage` -- the additive canary cell-rotation loop + off-chain coverage feed.
 *   - `startCanaryVerifyLoopAndClaims` -- the additive canary verify loop + (live mode) claims mTLS server.
 *
...
import { generateSessionKeypair, type NetworkConfig, type Logger, type RegistrationGauge } from '@dvconf/shared';
...
import { refreshDiscoveredValidators } from './validator-discovery.js';
...
/**
 * Auto-register (if needed), then generate + register + fund the session wallet.
 *
 * Exits the process on registration failure (see `ensureRegistered`'s doc comment), so
 * reaching the return always means registered=true.
 */
export async function registerAndSetupSessionWallet(args: {
  client: SuiClient;
  ...
  const { client, mainKeypair, config, log, graphqlClient, registrationGauge } = args;
  ...
  return { validatorCapId, sessionKeypair, sessionAddress, mainAddress };
}
 ...
 */
export function startCanaryCellLoopAndCoverage(args: {
  state: DaemonState;
  log: Logger;
  sessionAddress: string;
  validatorMinerId: string;
}): void {
  const { state, log, sessionAddress, validatorMinerId } = args;
  try {
 ...
 */
export async function startCanaryVerifyLoopAndClaims(args: {
  state: DaemonState;
  log: Logger;
  sessionAddress: string;
  sessionKeypair: Ed25519Keypair;
  ...
  const { state, log, sessionAddress, sessionKeypair, validatorMinerId, canaryMetrics } = args;
  try {
    ...
    state.canaryVerifyLoop = startCanaryVerifyLoop({
      ...
      deps: {
        ...
        onCoverageSample: canaryMetrics?.setCoverage,
        onQuorumSample: canaryMetrics?.setQuorumMet,
        onDivergencePromoted: canaryMetrics?.incDivergencePromoted,
      },
    });
