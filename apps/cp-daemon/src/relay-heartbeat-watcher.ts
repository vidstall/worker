...
// ── Live submitter/candidate-selector factories ───────────────────────────────
//
// Pure extraction to relay-heartbeat-submitters.ts (wired in cp-daemon
// index.ts / cp-watchers-wiring.ts). Re-exported here so existing import
// paths (`from './relay-heartbeat-watcher.js'`) keep working unchanged.
export {
  makeLiveReplacementCandidateSelector,
  makePromoteSubmitter,
  makeReplacementSubmitter,
} from './relay-heartbeat-submitters.js';
