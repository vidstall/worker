 ...
 *
 * ── MODULE LAYOUT (pure extract-module refactor; behavior unchanged) ────────────────────────────────
 * This file is now a thin CLI entry point. The actual logic lives under ./m2b/:
 *   m2b/common.ts          — shared paths, env-derived constants, run-log, small helpers.
 *   m2b/volume-io.ts        — read seeded daemon-keys/admin-creds off the booted-stack docker volume.
 *   m2b/capture-forward.ts  — the real browser → evil-relay → F1 pipe → validator sink capture leg.
 *   m2b/register-actors.ts  — register the 2 fresh validators + the fresh relay; resolve the proof.
 *   m2b/slash-queries.ts    — on-chain read queries (latest slash event, bond balance).
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import { Ed25519Keypair } from '@mysten/sui/keypairs/ed25519';
import { normalizeSuiAddress } from '@mysten/sui/utils';
import type { SlashCallOpts } from '../../apps/validator-daemon/src/canary/slash-submitter.ts';
import { submitCanarySlash } from '../../apps/validator-daemon/src/canary/slash-submitter.ts';
import {
  createSuiClient,
  loadNetworkConfig,
  createRoomWithRelay,
...
import { readDaemonKeysFromVolume, readAdminCredsFromVolume } from './m2b/volume-io.ts';
import { captureForwardedLeg } from './m2b/capture-forward.ts';
import { registerFreshValidatorWithSession, registerFreshRelay, resolveDivergenceProof, fundAddress } from './m2b/register-actors.ts';
import { queryLatestSlash, readBond } from './m2b/slash-queries.ts';
